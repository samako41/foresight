...
class AssetAssetReport(models.Model):
    _name = "ctx.account.asset.report"
    ...
    asset_id = fields.Many2one('ctx.account.asset', string='Asset', readonly=True)
    asset_category_id = fields.Many2one('ctx.account.asset.category', string='Asset category', readonly=True)
    ...
    def init(self):
        tools.drop_view_if_exists(self._cr, 'ctx_account_asset_report')
        self._cr.execute("""
            create or replace view ctx_account_asset_report as (
                select
                    min(dl.id) as id,
                    dl.name as name,
                ...
                from ctx_account_asset_depreciation_line dl
                    left join ctx_account_asset a on (dl.asset_id=a.id)
                    left join (select min(d.id) as id,ac.id as ac_id from ctx_account_asset_depreciation_line as d 
                    inner join ctx_account_asset as ac ON (ac.id=d.asset_id) group by ac_id) as dlmin on dlmin.ac_id=a.id
