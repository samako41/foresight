...
class CTXAssetModify(models.TransientModel):
    _name = 'ctx.asset.modify'
    ...
    def _get_asset_method_time(self):
        if self.env.context.get('active_id'):
            asset = self.env['ctx.account.asset'].browse(self.env.context.get('active_id'))
            self.asset_method_time = asset.method_time

    @api.model
    def default_get(self, fields):
        res = super(CTXAssetModify, self).default_get(fields)
        asset_id = self.env.context.get('active_id')
        asset = self.env['ctx.account.asset'].browse(asset_id)
        if 'name' in fields:
        ...
        if self.env.context.get('active_id'):
            active_asset = self.env['ctx.account.asset'].browse(self.env.context.get('active_id'))
    ...
    def modify(self):
        ...
        tracked_fields = self.env['ctx.account.asset'].fields_get(['method_number', 'method_period', 'method_end'])
