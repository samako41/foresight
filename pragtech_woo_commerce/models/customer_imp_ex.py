...
class Customer(models.Model):
    ...
    def cron_import_customer(self):
        ...
        if not all_instances:
            _logger.exception("There is WooCommerce instance found")
        else:
            for rec in all_instances:
                if rec:
            ...
            if data.status_code == 200:
                ...
                if parsed_data:
                    for ele in parsed_data:
                        customer = self.env['res.partner'].search([('woo_id', '=', ele.get('id'))], limit=1)
                        ...
                        if not customer and not customer_without_woo_id:
                            ...
                            if pro_create:
                                ...
                                if ele.get('shipping'):
                                    ...
                                    customer.create(dict_s)
                        else:
                            pro_create = customer.write(dict_p)
                            if pro_create:
                                ''' Search for updated customer '''
                                customer_record = self.env['res.partner'].search([('woo_id', '=', ele.get('id'))],limit=1)
                                if customer_record:
                                    ...
                                    if customer_id2:
                                        ...
                                        if ele.get('shipping'):
                                            ...
                                            customer_record.create(dict_ss)
