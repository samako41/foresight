...
class WooCoupon(models.Model):
    ...
    discount_specific_product_ids = fields.Many2many('product.product','product_loyalty_rel','product_id','loyalty_program_id',string="Products")
        ...
        if response.status_code == 200:
            ...
            for rec in parsed_data:
                ...
                if discount_type and discount_type == 'fixed_product':
                    ...
                    vals.update({
                        'discount_specific_product_ids': [(4, product.id, False) for product in product_list if product]
                    })

                coupon = self.env['loyalty.program'].search([('woo_id', '=', rec.get('id'))], limit=1)
                if not coupon:
    ...
    def export_selected_coupon(self, instance_id):
        ...
        selected_records = self.env['loyalty.program'].browse(selected_ids)

        if selected_records:
            records = selected_records
        else:
            records = self.env['loyalty.program'].search([('is_exported', '=', False)])
